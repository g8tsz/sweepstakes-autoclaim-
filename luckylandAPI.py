...
# ────────────────────────────────────────────
# Helper to run CDP activation in a worker thread
# ────────────────────────────────────────────

def _activate_cdp_mode_blocking(sb: SB, url: str):
    """Small wrapper so we can run activate_cdp_mode() in a thread."""
...
async def luckyland_uc(ctx, channel: discord.abc.Messageable):
    ...
    CDP activation is run in a worker thread to avoid
    ...
    # We’ll use the bot’s running event loop for run_in_executor
    ...
    try:
        ...
        with SB(uc=True, uc_cdp=True) as sb:
            ...
            # Run the CDP activation in a separate thread so its internal
            # asyncio.run() doesn’t clash with Discord’s event loop.
            await loop.run_in_executor(None, _activate_cdp_mode_blocking, sb, LOGIN_URL)
